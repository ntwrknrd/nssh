...
def _select_host_with_fzf(
    matches: Dict[str, str], query: Optional[str] = None
) -> HostMatch:
    ...
    try:
        [selection] = fzf_select(
            hostnames, "Select host:", exit_on_cancel=False, query=query
        )
    except FzfCancelled:
...
def main(argv: Sequence[str] | None = None):
    ...
    with timing_core.run_span("connect-workflow"):
        ...
        try:
            ...
            try:
                host_match = _select_host_with_fzf(exc.matches, query=search_term)
