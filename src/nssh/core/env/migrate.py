"""Migrate files from legacy locations to XDG-compliant paths."""
...
def default_config_root() -> Path:
    """Return the nssh config directory (~/.config/nssh)."""
    return default_config_path().parent


# Legacy paths -> (xdg_type, new_name)
# xdg_type: "state" = default_state_root(), "data" = default_data_root(), "config" = default_config_root()
LEGACY_MAPPINGS: List[Tuple[str, str, str]] = [
    ...
    ("~/.config/age/keys.txt", "config", "age.key"),
]
...
def migrate_legacy_files(*, prompt_overwrite: bool = True) -> List[Tuple[Path, Path]]:
    ...
    for legacy_path, xdg_type, new_name in LEGACY_MAPPINGS:
        ...
        elif xdg_type == "config":
            new = default_config_root() / new_name
        else:  # data
