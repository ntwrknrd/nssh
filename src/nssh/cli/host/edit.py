...
def _interactive_edit(
    ...
    console.print(f"\n[bold]Locating host '{final_hostname}'...[/bold]")
    ...
    show_panel(f"Host: {final_hostname}", syntax, style="cyan")

    # Collect changes
    changes: Dict[str, str] = {}

    console.print("\n[bold]=== SSH Config ===[/bold]")
    ...
    if current_identity and not yes:
        new_identity = ask_text(
            f"  IdentityFile [{current_identity}]",
    ...
    if not yes:
        update_password = confirm("  Update password?", default=False)
    ...
    try:
        ...
        if test_result["success"]:
            ...
            # Check if auth type needs updating
            ...
            if detected_method and detected_method != current_auth:
                console.print(f"[dim]Detected {detected_method} authentication[/dim]")
                if confirm(
...
def edit_command(
    ...
    with banner("EDIT SSH HOST", OK) as set_outcome:
        _interactive_edit(ctx, hostname, yes, dry_run, set_outcome)
