"""SCP benchmark capture command for nssh benchmark."""
...
def _create_test_file(size_kb: int) -> Path:
    """Create a temporary test file of specified size."""
    test_file = Path(tempfile.gettempdir()) / f"nssh-benchmark-test-{os.getpid()}.bin"
...
def _cleanup_test_file(test_file: Path) -> None:
...
def _save_scp_benchmark_archive(
    archive_dir: Path,
    ...
    """Save SCP benchmark output.txt, metadata.json, and create latest symlink."""
    ...
    # Create/update 'latest-scp' symlink
    latest_link = archive_dir.parent / "latest-scp"
    ...
    common.console.print(
        f"\n[green]SCP benchmark results archived to:[/green] {archive_dir}"
    )
    ...
    common.console.print(
        f"[dim]  - Latest SCP run: benchmark/latest-scp -> {archive_dir.name}"
    )
...
def _capture_scp_impl(
    ...
    test_file = _create_test_file(file_size_kb)

    try:
        # Handle both str (binary path) and list (dev mode command prefix)
        if isinstance(nssh_binary, list):
            base_cmd = nssh_binary + [
                "cp",
                str(test_file),
                f"{host}:/tmp/nssh-benchmark-test.bin",
            ]
        else:
            base_cmd = [
                nssh_binary,
        ...
        for idx in range(1, warmups + samples + 1):
            ...
            run_samples = timing_core.build_benchmark_samples(
                entries, default_run_id=idx
            )
            if not run_samples:
                common.console.print(
        ...
        common.console.print(f"[dim]Cleaned up test file: {test_file}[/dim]")
